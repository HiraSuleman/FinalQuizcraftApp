...
import com.example.quizcraftappfyp.adapter.StudentAccountAdapter;
...
public class StudentAccountActivity extends AppCompatActivity {
   ...
   StudentAccountAdapter adapter;
    FirebaseAuth mAuth;
    List<StudentAccountModel> list=new ArrayList<>();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        adapter=new StudentAccountAdapter(list,this);
        recyclerView.setHasFixedSize(true);
        recyclerView.setLayoutManager(new LinearLayoutManager(this));
        recyclerView.setAdapter(adapter);
        mAuth=FirebaseAuth.getInstance();
        final StudentAccountModel user=new StudentAccountModel();
    ...
    private void sigin(final StudentAccountModel user) {
        mAuth.signInWithEmailAndPassword(user.email,user.password).addOnCompleteListener(new OnCompleteListener<AuthResult>() {
            ...
            public void onComplete(@NonNull Task<AuthResult> task) {
                ...
                if(task.isSuccessful()){
                    ...
                    Toast.makeText(StudentAccountActivity.this, ""+task.getException(), Toast.LENGTH_SHORT).show();
                }
            }
        });
    }
